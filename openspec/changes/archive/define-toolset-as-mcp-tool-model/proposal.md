...
`openspec/specs/dispatch-core/spec.md` ("MCP tool method surface",
"`tools/call` uses the standard command path"),
`openspec/specs/toolset-lifecycle/spec.md` ("Toolset schema maps to
one MCP tool definition per toolset", "Dynamically-pushed toolsets run
out-of-process", "Verification gates registration regardless of push
convenience"), and `openspec/specs/capability-sync/spec.md`
("Distinct from MCP's live tool-change notification...", "Device
publishes its toolset list on session establishment"). This directory
is retained as historical record of the design reasoning; the base
spec files are the current source of truth going forward.
