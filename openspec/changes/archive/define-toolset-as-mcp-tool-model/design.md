...
**Follow-on, 2026-08-16 — confirmed, tracked separately:** this
decision doesn't say where descriptive per-method metadata
(`load_type`, `version`, `timeout_ms`, etc. — first needed by
`add-triage-skillset-mapping-phase1`) lives within a `tools/list`
entry, or whether the entry's detail is filtered by the caller's ACL
scope. Both surfaced while correcting that change and apply to every
toolset's `tools/list` entry, not just triage's —
`openspec/changes/resolve-tools-list-metadata-and-acl-scoping/`
(confirmed by direct instruction 2026-08-16) settles both: a sibling
`methods` field for metadata; two-tier ACL-scoped visibility (every
toolset named; per-toolset detail gated by `acl_policy_store_query()`).
Not folded into this decision directly since it's additive to the
`oneOf` shape defined here, not a revision of it — see that change's
